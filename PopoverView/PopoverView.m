...
#define kDividerColor [UIColor colorWithRed:0.329f green:0.341f blue:0.353f alpha:0.15f]
...
//normal text color
#define kTextColor [UIColor colorWithRed:0.329f green:0.341f blue:0.353f alpha:1]
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)inView withViewArray:(NSArray *)viewArray {
    ...
    [self showAtPoint:point inView:inView withContentView:container];
}

- (void)showAtPoint:(CGPoint)point inView:(UIView *)inView withTitle:(NSString *)title withViewArray:(NSArray *)viewArray {
    UIView *container = [[UIView alloc] initWithFrame:CGRectZero];
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray {
    ...
    for (NSString *string in stringArray) {
        ...
        [textButton setTitleColor:[UIColor colorWithRed:0.098f green:0.102f blue:0.106f alpha:1] forState:UIControlStateHighlighted];
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    BOOL found = NO;
    for (NSUInteger i = 0; i < subviewsArray.count && !found; i++) {
        ...
        if (CGRectContainsPoint(view.frame, point)) {
            ...
            if (delegate && [delegate respondsToSelector:@selector(popoverView:didSelectItemAtIndex:)]) {
                [delegate popoverView:self didSelectItemAtIndex:(NSInteger)i];
            }
...
- (void)didTapButton:(UIButton *)sender {
    ...
    if (delegate && [delegate respondsToSelector:@selector(popoverView:didSelectItemAtIndex:)]) {
        [delegate popoverView:self didSelectItemAtIndex:(NSInteger)index];
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (kDrawTitleGradient) {
        ...
        if (titleBGHeight > 0.f) {
            ...
            //These floats are the top and bottom offsets for the gradient drawing so the drawing includes the arrows.
            topOffset = (!above ? kArrowHeight : 0.f);
            ...
            [[UIColor colorWithRed:0.741f green:0.741f blue:0.741f alpha:0.5f] setFill];
            [dividerLine fill];
