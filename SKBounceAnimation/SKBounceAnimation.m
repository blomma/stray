	...
	if (self.fromValue && self.toValue && self.duration) {
		if ([self.fromValue isKindOfClass:[NSNumber class]] && [self.toValue isKindOfClass:[NSNumber class]]) {
			...
			if ([valueType rangeOfString:@"CGRect"].location == 1) {
                ...
                CGPathRef path = [self newPathFromXValues:[self valueArrayForStartValue:fromPoint.x endValue:toPoint.x]
                ...
                CGPathRef path = [self newPathFromXValues:[self valueArrayForStartValue:fromSize.width endValue:toSize.width]
...
- (CGPathRef)newPathFromXValues:(NSArray*)xValues yValues:(NSArray*)yValues {
...
- (NSArray *)valueArrayForStartValue:(CGFloat)startValue endValue:(CGFloat)endValue {
	NSUInteger steps = (NSUInteger)(60 * self.duration); //60 fps desired
	...
	for (NSUInteger t = 0; t < steps; t++) {
		...
		if (self.shake) {
			oscillationComponent =  (CGFloat)sin(omega*t);
		} else {
			oscillationComponent =  (CGFloat)cos(omega*t);
		}
		...
		value = coefficient * (CGFloat)pow(2.71, alpha*t) * oscillationComponent + endValue;
